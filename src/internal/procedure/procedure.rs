mod private {
        ...
        TMiddleware: MiddlewareBuilder,
    {
        fn build<'b>(
            ...
            let (kind, Marker(resolver, _)) = self
                .inner
                .take()
                .expect("Called 'DynProcedure.build()' in invalid state! This is likely a bug in rspc's types.");
...
pub(crate) use private::{MissingResolver, Procedure};

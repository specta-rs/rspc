...
For rspc to be able to convert your types into Typescript they must implement the `specta::Type` trait.
[Specta](https://github.com/oscartbeaumont/rspc/tree/main/specta) is a crate that was created so that rspc can introspect Rust types.
The `Type` trait allows the Typescript exporter to understand the fields, generics and dependant types of a Rust type.

To access `specta::Type`, add `specta` as a dependency:

```bash
cargo add specta
```

We have already implemented `specta::Type` for most in-built types.
If you find a missing one open a [GitHub Issue](https://github.com/oscartbeaumont/specta/issues/new).
